 ...
 * =====================================================
 *
 * Computes a φ-weighted coherence score for any triangle:
 * a center entity + exactly two connector entities.
 *
 * Three points:
 ...
 * (strong competitor = downward pressure on the center).
 *
 ...
 * @param {Map<string,number>} overrides  entityId → signal override
 ...
 *
 * @param {string}           centerId    – center entity ID
 * @param {string}           connA       – first connector entity ID
 * @param {string}           connB       – second connector entity ID
 * @param {Map<string,number>} [overrides] – optional signal overrides
 * @param {number}           [safetyLevel=0.618]
 * @returns {{
 *   coherenceScore: number,
 *   safetyTier: string,
 *   isOptimal: boolean,
 *   points: { p1: object, p2: object, p3: object },
 * }}
 */
function computeTriangle(centerId, connA, connB, overrides = new Map(), safetyLevel = SAFETY_LEVEL) {
  ...
  const coherenceScore = clamp01((s1 * W1 + s2 * W2 + s3 * W3) / W_TOTAL);
